...
#### gs=128 flip — full BEIR-6 vs gs=32 baseline (CPU 8-worker, full eval)

The full BEIR-6 (arguana / fiqa / nfcorpus / quora / scidocs / scifact)
re-validation at the new gs=128 default vs the prior gs=32 baseline.
Sourced from
[`reports/beir_2026q2_gs128/sweep_rroq158_gs128.jsonl`](../reports/beir_2026q2_gs128/sweep_rroq158_gs128.jsonl):

| Dataset  | NDCG@10 (gs=32) | NDCG@10 (gs=128) | ΔNDCG    | R@100 (gs=32) | R@100 (gs=128) | ΔR@100   | CPU p95 (gs=32) | CPU p95 (gs=128) | p95 ratio | B/tok ratio |
|----------|----------------:|-----------------:|---------:|--------------:|---------------:|---------:|----------------:|-----------------:|----------:|------------:|
| arguana  | 0.3410          | 0.3386           | **−0.0024** | 0.9450     | 0.9429         | −0.0021  | 576 ms          | 570 ms           | 0.99×     | 0.87×       |
| fiqa     | 0.4223          | 0.4260           | **+0.0037** | 0.7151     | 0.7118         | −0.0033  | 279 ms          | 285 ms           | 1.02×     | 0.87×       |
| nfcorpus | 0.3794          | 0.3797           | +0.0004  | 0.3342     | 0.3321         | −0.0021  | 286 ms          | 223 ms           | **0.78×** | 0.87×       |
| quora    | 0.9705          | 0.9712           | +0.0007  | 0.9990     | 0.9990         | 0.0000   | 99 ms           | 93 ms            | 0.94×     | 0.87×       |
| scidocs  | 0.1858          | 0.1857           | −0.0001  | 0.4300     | 0.4296         | −0.0004  | 310 ms          | 294 ms           | 0.95×     | 0.87×       |
| scifact  | 0.7386          | 0.7402           | +0.0016  | 0.9633     | 0.9633         | 0.0000   | 398 ms          | 392 ms           | 0.99×     | 0.87×       |
| **avg**  | **0.5063**      | **0.5069**       | **+0.0006** | **0.7311** | **0.7298**  | **−0.0013** | **325 ms**   | **310 ms**       | **0.95×** | **0.87×**   |

Headline across the full BEIR-6 sweep at the new SOTA default:

- **−13% storage** (40 vs 46 B/tok at dim=128 → 6.4× smaller than fp16
  vs 5.5× at gs=32) on every dataset.
- **NDCG@10 essentially flat** vs gs=32 (per-dataset Δ avg = +0.0006;
  worst case arguana −0.0024, best case fiqa +0.0037).
- **CPU p95 lower or equal** on every dataset (largest win:
  nfcorpus −22%; only +/−2% bump: fiqa +2%).
- GPU p95 stays within +5% on every dataset (within typical run-to-run
  noise on the small datasets).

Decision: gs=128 is **Pareto-equal in quality** to gs=32 with
strictly better storage and CPU latency on the BEIR-6 mean. Shipped
as the SOTA default in Phase 8. For the rare workload that needs the
arguana −0.0024 NDCG point back, pin `Rroq158Config(group_size=64)`
or `BuildConfig(rroq158_group_size=64)` — see
[guides/quantization-tuning.md](guides/quantization-tuning.md) for the
per-dim recipe and override guidance.

#### 4-codec × 6-dataset × 2-mode sweep (full BEIR-6 at gs=128 default)

The full sweep that backs the default is
...
<!-- BEIR_2026Q2_FULL_TABLE_BEGIN — measured 2026-04-20 from reports/beir_2026q2_gs128/sweep_combined.jsonl (full BEIR-6 sweep at the new SOTA gs=128 default + non-rroq158 cells from reports/beir_2026q2/sweep.jsonl post-Phase-7) -->

Encoder: lightonai/GTE-ModernColBERT-v1. CPU lane uses 8 native Rust workers.
Run `python scripts/format_beir_2026q2_table.py reports/beir_2026q2_gs128/sweep_combined.jsonl --format markdown`
to regenerate.
...
#### `rroq158` (default — Riemannian 1.58-bit ternary, K=8192, **group_size=128** SOTA)
...
| **rroq158 (gs=128 default)** | 0.5069 | 0.5373 | 0.5965 | 0.7298 | 4.8 | 309.6 | 249.9 | 43.4 |
...
verdict at the bottom of the rendered table. The Phase-8 result (full
BEIR-6 sweep at the new gs=128 default):
...
2. **rroq158 retention rule** (avg ΔNDCG@10 ≥ −1.5 pt and avg GPU p95
   ratio ≤ 1.2×): **ACCEPTED** at the new SOTA gs=128 default. avg
   ΔNDCG@10 = **−1.37 pt** (within the −1.50 pt budget; improved from
   −1.43 pt at the prior gs=32 default), avg GPU p95 ratio = **1.20×**
   (at the 1.20× budget), avg ΔR@100 = **−0.62 pt** (essentially flat
   in absolute terms). rroq158 at gs=128 stays the build-time default
   with rroq4_riem available as the opt-in no-quality-loss lane. Vs
   the prior gs=32 default, the gs=128 flip is +0.0006 NDCG@10
   per-dataset average (essentially Pareto-equal) at **−13% storage**
   and **−5% avg CPU p95** (325 ms → 310 ms), with the lower CPU p95
   delivered by one fewer scale load per group in the popcount kernel.
...
`reports/beir_2026q2/topk_overlap.jsonl`. **Note:** the brute-force
overlap diagnostic was measured at the prior gs=32 default; the gs=128
flip is essentially Pareto-equal at the production lane (NDCG@10 avg
+0.0006 / R@100 avg −0.0013 vs gs=32 — see the gs=128 vs gs=32
comparison table above), so the per-codec brute-force overlap is
expected to track within run-to-run noise. A refreshed brute-force
sweep at gs=128 will land in a follow-up commit if and when there is
a workload-driven need to re-quantify the displacement.
...
| `rroq158` (gs=128, **SOTA default**) | **~40 B** | −1.37 pt avg NDCG@10 vs FP16, max −2.93 pt (arguana); per-dataset Δ vs gs=32 avg = +0.0006 (essentially Pareto-equal) | **~6.4× smaller** | 0.78–1.02× CPU p95 vs gs=32 (avg 0.95×); GPU 1.20× FP16 (within budget) | shipped (default) |
| `rroq158` (gs=32, prev default) | ~46 B     | −1.43 pt avg NDCG@10 vs FP16, max −2.78 pt (arguana) | ~5.5× smaller | 1.13× GPU avg, ~3.15× slower CPU avg vs FP16 | shipped (override `Rroq158Config(group_size=32)`) |
