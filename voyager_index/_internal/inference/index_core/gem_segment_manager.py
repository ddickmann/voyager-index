"""
GEM Segment Manager — Manages sealed GEM graph segments
for multi-vector (ColBERT/ColPali) search.
"""
...
class GemSegmentManager:
    ...
    Wraps the Rust-backed ``GemSegment`` with a Pythonic interface for
    building, searching, persisting, and injecting semantic shortcuts.
    """
    ...
    def build(
        ...
        """Build a sealed GEM segment from concatenated token vectors."""
    ...
    def build_from_embeddings(
        ...
        """Build from a list of per-document (n_tokens, dim) matrices."""
    ...
    def search(
        ...
        """Search the sealed segment. Returns [(doc_id, score), ...]."""
    ...
    def inject_shortcuts(
        self,
        training_pairs: List[Tuple[np.ndarray, int]],
        max_shortcuts_per_node: int = 4,
    ):
        """Inject semantic shortcut edges from training data."""
        if not self.is_ready():
