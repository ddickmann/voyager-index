...
def test_pyproject_install_contract_matches_public_release_story() -> None:
    ...
    assert "latence-shard-engine>=0.1.3" in extras["shard-native"]
    assert "latence-solver>=0.1.3" in extras["solver"]
    assert "latence-shard-engine>=0.1.3" in extras["native"]
    assert "latence-solver>=0.1.3" in extras["native"]
